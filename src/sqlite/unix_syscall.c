...
int robustFchown(int fd, uid_t uid, gid_t gid) {
  return ((uid_t (*)(void))aSyscall[SQLITE_SYSCALL_GETEUID].pCurrent)()
             ? 0
             : ((int (*)(int, uid_t, gid_t))aSyscall[SQLITE_SYSCALL_FCHOWN].pCurrent)(fd, uid, gid);
}
...
int robust_open(const char *z, int f, mode_t m) {
  ...
  if (fd >= 0) {
    if (m != 0) {
      struct stat statbuf;
      if (((int (*)(int, struct stat *))aSyscall[SQLITE_SYSCALL_FSTAT].pCurrent)(fd, &statbuf) == 0 &&
          statbuf.st_size == 0 && (statbuf.st_mode & 0777) != m) {
...
int seekAndWriteFd(int fd, i64 iOff, const void *pBuf, int nBuf, int *piErrno) {
  ...
  do {
    rc = (int)((ssize_t (*)(int, const void *, size_t, off64_t))aSyscall[SQLITE_SYSCALL_PWRITE64].pCurrent)(fd, pBuf,
                                                                                                            nBuf, iOff);
  } while (rc < 0 && errno == EINTR);
...
static const char *unixTempFileDir(void) {
  ...
  while (1) {
    if (zDir != 0 && ((int (*)(const char *, struct stat *))aSyscall[SQLITE_SYSCALL_STAT].pCurrent)(zDir, &buf) == 0 &&
        S_ISDIR(buf.st_mode) && ((int (*)(const char *, int))aSyscall[SQLITE_SYSCALL_ACCESS].pCurrent)(zDir, 03) == 0) {
      return zDir;
    }
