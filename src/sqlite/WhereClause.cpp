...
void __attribute__((noinline)) sqlite3WhereAddLimit(WhereClause *pWC, Select *p) {
  if (p->pGroupBy == 0 && (p->selFlags & (0x0000001 | 0x0000008)) == 0 &&
    ...
    if (p->iOffset != 0 && (p->selFlags & 0x0000100) == 0) {
      whereAddLimitExpr(pWC, p->iOffset, p->pLimit->pRight, iCsr, SQLITE_INDEX_CONSTRAINT_OFFSET);
    }
    if (p->iOffset == 0 || (p->selFlags & 0x0000100) == 0) {
      whereAddLimitExpr(pWC, p->iLimit, p->pLimit->pLeft, iCsr, SQLITE_INDEX_CONSTRAINT_LIMIT);
    }
  }
}
    ...
    if (j < 0) {
      ...
      if (pLoop->maskSelf == pTerm->prereqAll) {
        if ((pTerm->eOperator & 0x3f) != 0 ||
            (pWC->pWInfo->pTabList->a[pLoop->iTab].fg.jointype & (0x08 | 0x40)) == 0) {
