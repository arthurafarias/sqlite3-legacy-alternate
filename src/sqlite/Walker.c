...
int sqlite3WalkExpr(Walker *pWalker, Expr *pExpr) { return pExpr ? sqlite3WalkExprNN(pWalker, pExpr) : 0; }
...
int sqlite3WalkSelectExpr(Walker *pWalker, Select *p) {
  ...
  if (p->pWinDefn) {
    Parse *pParse;
    if (pWalker->xSelectCallback2 == sqlite3WalkWinDefnDummyCallback || ((pParse = pWalker->pParse) != 0 && (pParse->eParseMode >= 2))
...
int resolveExprStep(Walker *pWalker, Expr *pExpr) {
  ...
  case 172: {
    ...
    if (pDef == 0) {
      ...
      if ((pDef->funcFlags & 0x000100000) || (((pExpr)->flags & (u32)(0x80000000)) != 0)) {
    ...
    if (0 == (pParse->eParseMode >= 2)) {
      ...
      if (pDef && pDef->xValue == 0 && pWin) {
        ...
        pNC->nNcErr++;
      } else if ((is_agg && (pNC->ncFlags & 0x000001) == 0) || (is_agg && (pDef->funcFlags & 0x00010000) && !pWin) || (is_agg && pWin && (pNC->ncFlags & 0x004000) == 0)) {
        const char *zType;
...
int gatherSelectWindowsSelectCallback(Walker *pWalker, Select *p) { return p == pWalker->u.pSelect ? 0 : 1; }
...
__attribute__((noinline)) int exprNodeIsConstantFunction(Walker *pWalker, Expr *pExpr) {
  ...
  if (pDef == 0 || pDef->xFinalize != 0 || (pDef->funcFlags & (0x0800 | 0x2000)) == 0 || (((pExpr)->flags & (u32)(0x1000000)) != 0)) {
    pWalker->eCode = 0;
    return 2;
  }
  return 1;
...
int exprNodeIsConstant(Walker *pWalker, Expr *pExpr) {
  ...
  case 172:
    if ((pWalker->eCode >= 4 || (((pExpr)->flags & (u32)(0x100000)) != 0)) && !(((pExpr)->flags & (u32)(0x1000000)) != 0)) {
      if (pWalker->eCode == 5)
        (pExpr)->flags |= (u32)(0x40000000);
      return 0;
...
int exprIdxCover(Walker *pWalker, Expr *pExpr) {
  if (pExpr->op == 168 && pExpr->iTable == pWalker->u.pIdxCover->iCur && sqlite3TableColumnToIndex(pWalker->u.pIdxCover->pIdx, pExpr->iColumn) < 0) {
    pWalker->eCode = 1;
    return 2;
  }
  return 0;
      ...
      if (i > mxTerm) {
        ...
        if (i >= 0) {
          ...
          if (pExpr->pLeft && (pItem->pFunc->funcFlags & 0x0020) == 0) {
            ...
            pItem->bUseSubtype = (pItem->pFunc->funcFlags & 0x000100000) != 0;
          } else {
            pItem->iOBTab = -1;
          }
...
int renameColumnExprCb(Walker *pWalker, Expr *pExpr) {
  ...
  } else if (pExpr->op == 168 && pExpr->iColumn == p->iCol && ((((pExpr)->flags & (0x1000000 | 0x2000000)) == 0)) && p->pTab == pExpr->y.pTab) {
    renameTokenFind(pWalker->pParse, p, (void *)pExpr);
  }
  return 0;
}
    ...
    if (pFix->bTemp == 0 && pItem->fg.isSubquery == 0) {
      if (pItem->fg.fixedSchema == 0 && pItem->u4.zDatabase != 0) {
        if (iDb != sqlite3FindDbName(db, pItem->u4.zDatabase)) {
          sqlite3ErrorMsg(pFix->pParse, "%s %T cannot reference objects in database %s", pFix->zType, pFix->pName, pItem->u4.zDatabase);
          return 2;
        }
...
int selectExpander(Walker *pWalker, Select *p) {
  ...
  if (pParse->pWith && (p->selFlags & 0x0200000)) {
    if (p->pWith == 0) {
      p->pWith = (With *)sqlite3DbMallocZero(db, (
  ...
  for (i = 0, pFrom = pTabList->a; i < pTabList->nSrc; i++, pFrom++) {
    ...
    if (pFrom->zName == 0) {
      ...
      if (!((pTab)->eTabType == 0)) {
        ...
        else if ((((pTab)->eTabType == 1)) && (pFrom->fg.fromDDL || (pParse->prepFlags & 0x20)) && (pTab->u.vtab.p != 0) && pTab->u.vtab.p->eVtabRisk > ((db->flags & 0x00000080) != 0)) {
      ...
      if (pE->op != 180 && (pE->op != 142 || pRight->op != 180)) {
        ...
        for (i = 0, pFrom = pTabList->a; i < pTabList->nSrc; i++, pFrom++) {
          ...
          for (j = 0; j < nAdd; j++) {
            ...
            if ((pTabList->nSrc > 1 && ((pFrom->fg.jointype & 0x40) == 0 || (selFlags & 0x0000800) != 0 || !inAnyUsingClause(zName, pFrom, pTabList->nSrc - i - 1))) || (pParse->eParseMode >= 2)) {
              Expr *pLeft;
            ...
            if ((selFlags & 0x0000800) != 0 && !(pParse->eParseMode >= 2)) {
              ...
              if ((pFrom->fg.isUsing && sqlite3IdListIndex(pFrom->u3.pUsing, zName) >= 0) || (pUsing && sqlite3IdListIndex(pUsing, zName) >= 0) || (j < pTab->nCol && (pTab->aCol[j].colFlags & 0x0400))) {
                pX->fg.bNoExpand = 1;
              }
            } else if (longNames) {
...
int havingToWhereExprCb(Walker *pWalker, Expr *pExpr) {
  if (pExpr->op != 44) {
    ...
    if (sqlite3ExprIsConstantOrGroupBy(pWalker->pParse, pExpr, pS->pGroupBy) && (((pExpr)->flags & (0x000001 | 0x20000000)) == 0x20000000) == 0 && pExpr->pAggInfo == 0) {
    ...
    do {
      ...
      if (ii < nSrc) {
        if (pCtx->iJoin && iTab > pCtx->iJoin) {
          sqlite3ErrorMsg(pWalker->pParse, "%s references tables to its right", (pCtx->bFuncArg ? "table-function argument" : "ON clause"));
          return 2;
        }
        break;
      }
